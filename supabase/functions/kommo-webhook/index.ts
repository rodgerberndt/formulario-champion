...
function getMessageVariant(estagio?: string): string {
  ...
  if (
    s.includes("pré-escala") || s.includes("pre-escala") ||
    s.includes("vendas constantes")
  ) return "VAR_PRE_ESCALA";
  if (
    s.includes("validação") || s.includes("validacao") ||
    s.includes("primeiras vendas")
  ) return "VAR_VALIDACAO";
...
async function writeLog(
  ...
  try {
    await supabase.from("kommo_webhook_logs").insert({
      ...
      stage: entry.stage,
      source: entry.source,
      request_payload: maskSecrets(entry.request_payload) as Record<
        string,
        unknown
      >,
      response_payload: maskSecrets(entry.response_payload) as Record<
        string,
        unknown
      >,
      status: entry.status,
...
async function updateLeadKommoStatus(
  ...
  for (let attempt = 1; attempt <= 8; attempt++) {
    ...
    if (updateError) {
      console.warn(
        `[kommo-webhook] Lead status update failed on attempt ${attempt}:`,
        updateError,
      );
    }
  ...
  console.warn(
    `[kommo-webhook] Could not update lead ${leadDbId} after retries`,
  );
}
...
async function waitForLeadRow(
  ...
  {
    capi_events_sent: Record<string, boolean> | null;
    tier: string | null;
...
> {
  for (let attempt = 1; attempt <= 8; attempt++) {
    ...
    if (error) {
      console.warn(
        `[kommo-webhook] Lead fetch failed on attempt ${attempt}:`,
        error,
      );
    }
    ...
    if (data) {
      return {
        capi_events_sent:
          (data.capi_events_sent as Record<string, boolean> | null) || {},
        tier: data.tier,
...
async function searchContact(phone: string): Promise<{ id: number } | null> {
  const url = `https://${KOMMO_SUBDOMAIN}.kommo.com/api/v4/contacts?query=${
...
async function createContact(
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`Contact creation failed (${res.status}): ${text}`);
  }
...
function buildTags(leadData: Record<string, unknown>): Array<{ name: string }> {
  ...
  if (leadData.estagio_negocio) {
    tags.push({
      name: `stage_${sanitizeTag(String(leadData.estagio_negocio))}`,
    });
  }
  if (leadData.mercado) {
...
async function createLead(
  ...
  const messageVariant = getMessageVariant(
    leadData.estagio_negocio as string | undefined,
  );
  ...
  console.log(
    `[create_lead] Creating lead: ${name} | pipeline=${KOMMO_PIPELINE_ID} status=${KOMMO_STATUS_ID} variant=${messageVariant} tags=${
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isValidWebhook = webhookSecret && INTERNAL_WEBHOOK_SECRET &&
      webhookSecret === INTERNAL_WEBHOOK_SECRET;
    ...
    const authSource = isValidWebhook
      ? "webhook_secret"
      : isValidAdmin
      ? "admin_token"
      : "gateway";
    ...
    console.log(
      `[kommo-webhook] Processing lead: ${leadData.nome_completo} | source: ${source}`,
    );
    ...
    if (!leadData.whatsapp || !isValidPhone(String(leadData.whatsapp))) {
      const errMsg =
        "PENDING_PHONE: whatsapp missing or invalid – lead saved locally but NOT sent to Kommo";
      console.error(errMsg);
      await writeLog(supabase, {
        lead_name: leadData.nome_completo,
        lead_phone: leadData.whatsapp,
      ...
      return new Response(
        JSON.stringify({ error: errMsg, status: "PENDING_PHONE" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (!KOMMO_API_KEY || !KOMMO_SUBDOMAIN) {
      const errMsg =
        "Kommo credentials not configured (API_KEY or SUBDOMAIN missing)";
      console.error(errMsg);
      await writeLog(supabase, {
        lead_name: leadData.nome_completo,
        lead_phone: leadData.whatsapp,
    ...
    try {
      ...
      if (existing) {
        ...
        await writeLog(supabase, {
          ...
          request_payload: { phone, action: "found_existing" },
        });
      } else {
        ...
        await writeLog(supabase, {
          ...
          final_status: "IN_PROGRESS",
          contact_id: contactId,
          request_payload: { phone, action: "created_new" },
        });
      }
    } catch (err) {
    ...
    if (leadDbId) {
      try {
        ...
        console.log(
          `[kommo-webhook] CAPI URL: ${capiUrl}, has webhook secret: ${!!webhookSecret}, has service key: ${!!serviceKey}`,
        );
        ...
        if (!capiSent["CompleteRegistration"]) {
          ...
          try {
            ...
            console.log(
              `[kommo-webhook] CAPI CompleteRegistration status=${res.status}:`,
              resultText,
            );
            if (res.ok) capiSent["CompleteRegistration"] = true;
          } catch (e) {
            console.error(
              "[kommo-webhook] CAPI CompleteRegistration error:",
              e,
            );
          }
        }
        ...
        if (!capiSent[tierEventName] && tier !== "Desqualificado") {
          ...
          try {
            const res = await fetch(capiUrl, {
              ...
              body: JSON.stringify({
                lead_id: leadDbId,
                event_name: tierEventName,
              }),
            });
            const resultText = await res.text();
            console.log(
              `[kommo-webhook] CAPI ${tierEventName} status=${res.status}:`,
              resultText,
            );
        ...
        if (!capiSent["MQL"] && isMql) {
          ...
          try {
            ...
            const resultText = await res.text();
            console.log(
              `[kommo-webhook] CAPI MQL status=${res.status}:`,
              resultText,
            );
        ...
        if (eventsToSend.length > 0) {
          await supabase
            ...
            .eq("id", leadDbId);
          console.log(
            `[kommo-webhook] CAPI events sent: ${eventsToSend.join(", ")}`,
          );
        }
      } catch (capiErr) {
    ...
    console.log(
      `[kommo-webhook] SUCCESS - Contact: ${contactId}, Lead: ${kommoLeadId}, Variant: ${messageVariant}`,
    );
