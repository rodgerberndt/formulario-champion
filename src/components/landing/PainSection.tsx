...
export function PainSection() {
  ...
  return (
    <section className="py-12 md:py-20 relative overflow-hidden bg-foreground" ref={ref}>
      <div className="container mx-auto px-5 max-w-5xl relative z-10">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {pains.map((pain, i) => (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-destructive/15 border border-destructive/30 flex items-center justify-center mb-3">
                <pain.icon className="w-5 h-5 text-destructive" />
              </div>
              <h3 className="text-background mb-1.5" style={{ fontSize: "1.25rem" }}>
                {pain.title}
              </h3>
              <p className="text-sm text-background/60 leading-relaxed">{pain.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
